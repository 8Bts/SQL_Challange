...
SELECT game.team1, game.team2, goal.player FROm game
 JOIN goal ON (id=matchid) WHERE goal.player LIKE 'Mario%';
...
SELECT game.mdate, eteam.teamname FROM game JOIN eteam ON (team1=eteam.id)
 WHERE game.team1 = (SELECT eteam.id FROM eteam WHERE coach = 'Fernando Santos');
...
SELECT mdate,
  ...
  FROM game LEFT JOIN goal ON matchid = id GROUP BY game.id ORDER BY mdate, matchid, team1, team2;